...
class Blip_Synth {
	static_assert( 1 <= quality && quality <= 5, "quality must be between 1 and 5");
	static_assert( -32768 <= range && range <= 32767, "range must be in range of a short");
	enum {
		...
		fine_bits = (
		        fine_mode ?
		                (abs_range <= 64 ? 2 : abs_range <= 128 ? 3 : abs_range <= 256 ? 4 : abs_range <= 512 ? 5 :
		                 abs_range <= 1024 ? 6 : abs_range <= 2048 ? 7 : 8) :
		                0)
	};
